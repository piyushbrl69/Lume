...
export default function VocabList({ words, onPlayAudio }: VocabListProps) {
  ...
  if (words.length === 0) {
    return (
      <div className="text-center py-12 px-4 text-slate-500 italic border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
      ...
      {Object.entries(groupedWords).map(([date, dateWords]) => (
        <div key={date} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {dateWords.map(word => (
              <div key={word.id} className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start gap-3 hover:border-indigo-100 dark:hover:border-indigo-900/50 transition-colors">
                ...
                <button onClick={() => onPlayAudio(word.term)} className="p-3 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-slate-800 rounded-full transition-colors shrink-0 self-end sm:self-start sm:ml-4">
                  <Volume2 size={24} />
                </button>
              </div>
            ))}
