...
export default function FlashcardsPage() {
  ...
  return (
    <main className="p-8 max-w-5xl mx-auto min-h-screen bg-slate-50 dark:bg-slate-950">
      
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Active Recall</h1>
          <p className="text-slate-500 mt-2">Master your topics through spaced repetition.</p>
        </div>
        ...
        <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-xl">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors ${mode === 'study' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
          >
            <BookOpen size={18} /> Study
          </button>
          <button 
            onClick={() => setMode('manage')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors ${mode === 'manage' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
          >
            <Settings size={18} /> Manage Deck
          </button>
        </div>
      </header>
      ...
      {mode === 'manage' && (
        <section className="space-y-8">
          {/* Create Form */}
          <form onSubmit={handleAddCard} className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-4">
            <textarea 
              ...
              {deck.map((card) => (
                <div key={card.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-3 relative">
                  ...
                  <div>
                    ...
                    <p className="text-slate-800 dark:text-slate-200 font-medium pr-16">{card.front}</p>
                  </div>
                  <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Back</span>
                    <p className="text-slate-600 dark:text-slate-400">{card.back}</p>
                  </div>
                  <div className="mt-auto pt-2 text-xs text-slate-400 font-medium flex gap-4">
