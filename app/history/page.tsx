  ...
  return (
    <main className="p-8 max-w-4xl mx-auto min-h-screen">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Completed History</h1>
        <p className="text-slate-500 mt-2">Review your past missions and accomplishments.</p>
      </header>

      {sortedDates.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center">
          <Calendar className="text-slate-300 dark:text-slate-700 w-16 h-16 mb-4" />
          <p className="text-slate-500 font-medium">No completed tasks yet. Keep pushing!</p>
        </div>
      ) : (
        <div className="space-y-6">
          {sortedDates.map(date => (
            <div key={date} className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
              ...
              <ul className="space-y-4">
                {groupedTasks[date].map(task => (
                  <li key={task.id} className="flex items-center gap-4 text-slate-700 dark:text-slate-300">
